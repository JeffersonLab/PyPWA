...
from PyPWA.plugins.data.sv import s_process
...
@pytest.fixture
def full_data():
    parser = s_process.SvMemory()
    return parser.parse(CSV_TEST_DATA_2, numpy.float64)
...
def return_parsed_data(request):
    parser = s_process.SvMemory()
    return parser.parse(request.param, numpy.float64)
...
def looping_parser_test_data(numpy_flat, request):
    ...
    new_data = parser.parse(request.param, numpy.float64)
