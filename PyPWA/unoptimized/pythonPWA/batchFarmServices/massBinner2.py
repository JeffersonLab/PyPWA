...
"""
This is the PyPWA mass binning utility that simulation/fitting install
uses to bin .gamp files in mass.
"""
...
class MassBinner(object):
    ...
    def __init__(self,indir=None,bindir=None,gfile=None,verb="q"):
        ...
        Args:
            ...
            bindir (string): The full file path to the directory where the
                new mass bin directories will be written.
            gfile (string): The name of the .gamp file to be binned.
                (/Without/ the .gamp extension.)
            verb (string): either "q" for quiet or "v" for verbose.
        """

        self.indir = indir
        self.bindir = bindir
        self.Control = numpy.load(os.path.join(sys.argv[1], "GUI", "Control_List.npy"))
    ...
    def calcMass(self,event):
        ...
        if mass.dot(mass) >= 0.0:
            return numpy.sqrt(mass.dot(mass))
    ...
    def writeEvent(self,dataSlice):
        ...
        Args:
            dataSlice (numpy ndarray): The 2 densional array of a single event
                fron the _events 3D array.
    ...
    def fill(self,direct):
        ...
        if os.path.isfile(self.Qfile) and direct == "data":
            Qlist = numpy.loadtxt(self.Qfile)
        elif os.path.isfile(self.pFfile) and direct == "flat":
            pFlist = numpy.loadtxt(self.pFfile)
                ...
                if not os.path.isdir(os.path.join(self.bindir,str(int(self.Control[2]) + (b * int(self.Control[4])))+"_MeV")):
                    ...
                    if self.verb == "v" and b !=0:
                        print("Writing "+str(int(self.Control[2]) + (b * int(
                            self.Control[4])))+"_MeV"+" directories.")
        if os.path.isfile(self.Qfile) and direct == "data":
            nums = numpy.zeros(shape=[self.nBins])
