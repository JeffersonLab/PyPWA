...
class MemoryInterface(object):
    ...
    def parse(self, file_location):
        ...
        tester = PyPWA.libs.data.data_tools.DataTypeSearch()
        data_type = tester.search(file_location)

        if data_type == "kv":
            reader = PyPWA.libs.data.memory_wrapper.Kv()
        elif data_type == "sv":
            reader = PyPWA.libs.data.memory_wrapper.Sv()
        elif data_type == "yaml":
            reader = PyPWA.libs.data.memory_wrapper.Yaml()
        elif data_type == "pwa":
            reader = PyPWA.libs.data.memory_wrapper.Binary()
        else:
    ...
    def write(self, file_location, the_data):
        ...
        Args:
            ...
            the_data (object): The data that needs to be
                written to disk.
        """
        data_type = PyPWA.libs.data.data_tools.DataTypeWrite.search(file_location)

        if data_type == "sv":
            writer = PyPWA.libs.data.memory_wrapper.Sv()
        elif data_type == "yaml":
            writer = PyPWA.libs.data.memory_wrapper.Yaml()
        elif data_type == "pwa":
            writer = PyPWA.libs.data.memory_wrapper.Binary()
        elif data_type == "kv":
            writer = PyPWA.libs.data.memory_wrapper.Kv()
