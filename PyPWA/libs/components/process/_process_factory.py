...
from PyPWA.libs.components.process import _connection_factory
...
class _ProcessFactory(object):
    ...
    def __init__(
        ...
        self.__count = 0
        self.__kernels = None  # type: List[templates.Kernel]
    ...
    def build(self, kernels):
        # type: (List[templates.Kernel]) -> abstract_return
    ...
    def __set_basic_details(self, kernels):
        # type: (List[templates.Kernel]) -> None
...
def simplex_build(process_kernels):
    # type: (List[templates.Kernel]) -> simplex_return
...
def duplex_build(process_kernels):
    # type: (List[templates.Kernel]) -> duplex_return
    factory = _ProcessFactory(
